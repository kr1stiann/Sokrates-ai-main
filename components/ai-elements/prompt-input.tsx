...
export function PromptInputAttachment({
  ...
  const attachmentLabel = filename || (isImage ? "Bild" : "Bilaga");
        ...
        <div className="w-auto space-y-3">
          {isImage && (
            <div className="flex max-h-96 w-96 items-center justify-center overflow-hidden rounded-md border">
              {/* biome-ignore lint/performance/noImgElement: dynamic user uploads */}
              <img
                alt={filename || "bilaga förhandsvisning"}
...
export const PromptInputActionAddAttachments = ({
  label = "Lägg till bilder eller filer",
  ...props
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      ...
      if (incoming.length && accepted.length === 0) {
        onError?.({
          code: "accept",
          message: "Inga filer matchar de accepterade typerna.",
        });
        return;
      }
      ...
      if (accepted.length > 0 && sized.length === 0) {
        onError?.({
          code: "max_file_size",
          message: "Alla filer överskrider den maximala storleken.",
        });
        return;
      }
      ...
      setItems((prev) => {
        ...
        if (typeof capacity === "number" && sized.length > capacity) {
          onError?.({
            code: "max_files",
            message: "För många filer. Vissa lades inte till.",
          });
        }
